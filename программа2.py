...
def field_pattern(theta):
    """Характеристика направленности по полю (формула 1)"""
...
# Расчет Dmax через интеграл (формула 2)
...
# --- Чтение данных из файла 'расчётные_данные.txt' ---
sim_theta_deg = []
sim_d_db = []

try:
    with open('расчётные_данные.txt', 'r') as f:
        lines = f.readlines()
        data_started = False
        for line in lines:
            if '---' in line:
            ...
            if data_started and line.strip():
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        sim_d_db.append(float(parts[2]))
                    except ValueError: continue
except FileNotFoundError:
    print("Ошибка: файл 'расчётные_данные.txt' не найден.")

sim_theta_rad = np.radians(sim_theta_deg)
sim_d_linear = 10**(np.array(sim_d_db) / 10)

# --- Построение графиков ---
theta_plot = np.linspace(0, np.pi, 500)
d_analytic_linear = np.array([d_max_linear * (F_norm(t)**2) for t in theta_plot])
d_analytic_db = 10 * np.log10(np.clip(d_analytic_linear, 1e-6, None))

fig, axs = plt.subplots(2, 2, figsize=(14, 10))
plt.subplots_adjust(hspace=0.35, wspace=0.25)

# 1. Декартова (разы)
axs[0, 0].plot(np.degrees(theta_plot), d_analytic_linear, 'b-', label='Аналитика')
axs[0, 0].plot(sim_theta_deg, sim_linear, 'r--', label='Моделирование')
axs[0, 0].set_title('КНД в разах (Декартова)')
axs[0, 0].set_xlabel('Угол theta, град.')
axs[0, 0].grid(True); axs[0, 0].legend()

# 2. Декартова (дБ)
axs[0, 1].plot(np.degrees(theta_plot), d_analytic_db, 'b-', label='Аналитика')
axs[0, 1].plot(sim_theta_deg, sim_d_db, 'r--', label='Моделирование')
axs[0, 1].set_title('КНД в дБ (Декартова)')
axs[0, 1].set_ylim([-20, d_max_db + 2])
axs[0, 1].set_xlabel('Угол theta, град.')
axs[0, 1].grid(True); axs[0, 1].legend()

# 3. Полярная (разы)
ax_pol1 = plt.subplot(2, 2, 3, projection='polar')
ax_pol1.plot(theta_plot, d_analytic_linear, 'b-')
ax_pol1.plot(sim_theta_rad, sim_d_linear, 'r--')
...
# Смещение для отображения дБ в полярной сетке
offset = 40 
ax_pol2.plot(theta_plot, d_analytic_db + offset, 'b-', label='Аналитика')
ax_pol2.plot(sim_theta_rad, np.array(sim_d_db) + offset, 'r--', label='Моделирование')
ax_pol2.set_theta_zero_location('N')
ax_pol2.set_theta_direction(-1)
ax_pol2.set_yticklabels([]) # Скрываем шкалу со смещением
